...
def activate(input_layer, act='relu', name='activation'):
    if act is None:
        return input_layer
    if act == 'relu':
        return nn.relu(input_layer, name)
    if act == 'sqr':
        return square(input_layer, name)
    if act == 'sqr_sigmoid':
        return nn.sigmoid(square(input_layer, name))
    if act == 'sigmoid':
        return nn.sigmoid(input_layer, name)
    if act == 'softmax':
        return nn.softmax(input_layer,)
